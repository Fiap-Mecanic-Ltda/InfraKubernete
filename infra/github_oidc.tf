...
data "aws_iam_policy_document" "github_actions_assume_role" {
  statement {
    ...
    # Restringe a role aos workflows dos repositórios autorizados rodando na
    # branch main (push/workflow_dispatch) OU em jobs que referenciam o
    # Environment "production" (o GitHub troca o formato do sub claim nesse caso).
    condition {
      test     = "StringLike"
...
data "aws_iam_policy_document" "github_actions_ecr_push" {
  ...
  statement {
    ...
    actions = [
      ...
      "ecr:PutImage",
      # O deploy (repo de infra) descobre sozinho a tag da última imagem
      # publicada pelo repo da aplicação, em vez de depender de "latest" —
      # os Deployments usam imagePullPolicy IfNotPresent.
      "ecr:DescribeImages",
      "ecr:ListImages",
    ]
